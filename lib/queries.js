...
/**
 * Retrieve the reference regions (nl. referentieregio's) covered by a scope of
 * operation. The reference regions are derived from the scope by walking down to
 * its constituent municipalities and then up to each municipality's containing
 * reference region, returning the distinct set. This handles every shape of
 * scope: a single municipality, a district, a province, or a composed scope
 * (nl. "samengesteld werkingsgebied"). An organization can thus cover multiple
 * reference regions.
 * @param {string} scope - The URI of the scope (werkingsgebied) location.
 * @returns {Promise<{uri: string, uuid: string, label: string}[]>} The distinct
 *     reference regions covered by the provided scope.
 */
export async function getReferentieregiosForScope(scope) {
  const select = `
    ${PREFIX.prov}
    ${PREFIX.mu}
    ${PREFIX.geo}
    ...
    SELECT DISTINCT ?uri ?uuid ?label
    WHERE {
      GRAPH ${sparqlEscapeUri(locationGraph)} {
        ?uri mu:uuid ?uuid ;
             rdfs:label ?label ;
             ext:werkingsgebiedNiveau ?level .
        # NOTE: only the "Referentieregio" level value carries an @nl language
        # tag in the data (the other levels are plain literals), so compare on
        # the string value to stay tag-agnostic.
        FILTER (STR(?level) = "Referentieregio")
        {
          # the scope is itself a municipality, contained in the region
          ${sparqlEscapeUri(scope)} geo:sfWithin ?uri .
        } UNION {
          # the scope contains municipalities (a composed scope or a province)
          ?municipality geo:sfWithin ${sparqlEscapeUri(scope)} ;
                        ext:werkingsgebiedNiveau "Gemeente" ;
                        geo:sfWithin ?uri .
        } UNION {
          # the scope is a district, contained in a municipality
          ${sparqlEscapeUri(scope)} geo:sfWithin ?municipality .
          ?municipality ext:werkingsgebiedNiveau "Gemeente" ;
                        geo:sfWithin ?uri .
        }
      }
    }
